...
// =====================================
// A collaborative game development framework with a state machine-based menu
// system.
//
// KEY CONCEPTS:
// - State Machine: Menu system manages navigation between main menu and 3 games
// - Event-Driven Architecture: BT2/BT3 buttons use interrupt callback for fast
// response
// - Debouncing: Software debouncing to handle button noise
// - Modular Design: Each game (game_1, game_2, game_3) is independent with own
// loop
...
static void Run_Selected_Game(int game_index) {
  ...
  while (!game->should_exit()) {
    ...
    if (frame_time < 16) {
      HAL_Delay(16 - frame_time); // Cap at ~60 FPS (16 ms per frame)
    }
  }
  game->shutdown();
}
