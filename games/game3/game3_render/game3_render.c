...
// CAMERA-AWARE RECT DRAW

// Translates a world-space rect into screen-space, clips to the screen, and draws.
static void Game3_Render_Draw_Rect_Camera(int16_t world_x, int16_t world_y, uint8_t width, uint8_t height, uint8_t colour, uint8_t fill, const Game3_Camera *camera) {
    ...
    // Trim the part that falls off the left/top edge before drawing
    if (screen_x < 0) {
        width += screen_x;
        screen_x = 0;
    }
...
// Flash red while in damage-flash window, otherwise white
...
static void Game3_Render_Draw_Enemy_Health_Bar(const Game3_Enemy *enemy, const Game3_Camera *camera) {
    ...
    // Draw the empty bar background, then overlay the filled portion
